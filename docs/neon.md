...
The runtime client uses `prepare: false`, `fetch_types: false`, `max: 3`, `connect_timeout: 5`, `idle_timeout: 20`, and `max_lifetime: 60`. It pings with a 2s client-side timeout and recycles a stale postgres.js client instead of hanging. Concurrent callers share one ping; a ping timeout does not `end()` a client that already has in-flight queries. During `next build` the ping and lifetime caps are skipped so prerender cannot recycle a busy pool. `statement_timeout` / `lock_timeout` are set only on local/direct connections; they are not a reliable session setting through the transaction pooler.
