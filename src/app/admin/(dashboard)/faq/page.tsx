...
export default async function AdminFaqPage({ searchParams }: FaqPageProps) {
  ...
  let faqItems: Awaited<ReturnType<typeof listFaqsForSite>> = [];

  if (activeSite && !loadError) {
    try {
      faqItems = await listFaqsForSite(activeSite.id);
    } catch (error) {
      loadError =
        error instanceof Error
          ? error.message
          : "Could not load FAQs from the database.";
    }
  }
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-slate-600">
        Manage frequently asked questions per site.
      </p>
      ...
      {!loadError && activeSite && (
        <div className="mt-8">
          <FaqManager siteId={activeSite.id} faqs={faqItems} />
        </div>
      )}
    </div>
  );
